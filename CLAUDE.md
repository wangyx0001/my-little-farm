...
**Money/economy** is centralized in `PRICES` (sale value per resource type — `wood`/`wheat`/`berry`/`egg`/`milk`/`fish`) and each `ZONES` entry's `cost`. `cap()` and `speed()` derive the player's carry capacity and move speed from which upgrades (`S.un.*`) are unlocked, rather than storing them as separate stateful fields — check these two functions before assuming a stat lives on `S.player`.
